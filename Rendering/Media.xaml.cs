...
namespace XiboClient.Rendering
{
    ...
    public partial class Media : UserControl
    {
        ...
        /// <returns></returns>
        /// <summary>
        /// ?§Ï†ï???ÅÏÉÅ ?åÎçîÎß??îÏßÑ??libmpv?∏Ï? ?ïÏù∏.
        /// OptionsForm??Video Engine ?úÎ°≠?§Ïö¥ ?†ÌÉùÍ∞íÏùÑ Í∏∞Ï??ºÎ°ú ?êÎã®.
        /// </summary>
        /// <returns>libmpv ?îÏßÑ???†ÌÉù??Í≤ΩÏö∞ true, WindowsMediaPlayer??Í≤ΩÏö∞ false</returns>
        ...
        public static Media Create(MediaOptions options)
        {
            ...
            switch (options.type)
            {
                ...
                case "video":
                    options.uri = ApplicationSettings.Default.LibraryPath + @"\" + options.uri;
                    // libmpv ?îÏßÑ?¥Î©¥ VideoHwnd, ?ÑÎãàÎ©?Í∏∞Ï°¥ WPF MediaElement Í∏∞Î∞ò Video ?¨Ïö©
                    if (IsLibMpv())
